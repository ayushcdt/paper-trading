...
echo [4/5] Installing Artha-Recalibrate (monthly, 1st at 18:00)...
schtasks /Create /F /TN "Artha-Recalibrate" /TR "%RECAL%" /SC MONTHLY /D 1 /ST 18:00 /RL HIGHEST

echo [5/6] Installing Artha-MarkToMarket (every 15 min from 09:15)...
schtasks /Create /F /TN "Artha-MarkToMarket" /TR "c:\trading\scheduler\run_mark_to_market.bat" /SC MINUTE /MO 15 /ST 09:15 /RL HIGHEST

echo [6/6] Installing Artha-DailySummary (15:35 daily, EOD Telegram)...
schtasks /Create /F /TN "Artha-DailySummary" /TR "c:\trading\scheduler\run_daily_summary.bat" /SC DAILY /ST 15:35 /RL HIGHEST
